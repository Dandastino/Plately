...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    // Check the token when the website start
        ...
        try {
          ...
          if (userId) {
            ...
            if (storedUserData) {
              ...
              // Se i dati utente non sono in localStorage, prova a ottenerli dall'API
              try {
  ...
  const login = async (username, password) => {
    try {
      // Modifica qui: ottieni tutti gli utenti e filtra sul frontend
      const response = await axios.get(`http://localhost:3000/users`);
      
      // Cerca l'utente con username corrispondente (case insensitive)
